...
def main(file_path):
    ...
    # Load the models
    model = pickle.load(open("models/amazon_pred_word2vec.sav", "rb"))
    rf = pickle.load(open("models/amazon_pred_senti.sav", "rb"))
    ...
    count_greater_than_0_5 = sum(1 for p in predictions if p >= 2)
    count_less_than_0_5 = sum(1 for p in predictions if p < 2)
